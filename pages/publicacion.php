...
<head>
  ...
  require_once($_SERVER['DOCUMENT_ROOT'] . "/utils/get/getAllAdmins.php");
      ...
      if(isset($_GET["denuncia"])){
        ...
        if(in_array($_SESSION["id"],$admins)){
          ...
          echo renderPublicacionExtendida(
            ...
            $publicacion['publicacion_volumen'],
            $publicacion["publicacion_nombreRecibe"],
            $publicacion["publicacion_telefono"],
            $ubicaciones->origen,
            $ubicaciones->destino,
            $imagenes,
        ...
        $res=false;
        if($tmpStmt->execute()){
          $res=$tmpStmt->fetch(PDO::FETCH_ASSOC);
        }
        $tmpDB=null;
        $tmpConexion=null;
        $tmpSql=null;
