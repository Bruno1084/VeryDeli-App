...
function enviarNotificacion($idUsuario_destino,$mensaje){
    if(isset($db)){
        $db=null;   
    }
    $db=new DB();
    $connection=$db->getConnection();
    $sql="INSERT INTO `notificaciones` (`notificacion_mensaje`, `notificacion_estado`, `usuario_id`) VALUES ( ? , '0', ? )";

    $stmt=$connection->prepare($sql);

    $stmt->bindParam(1,$mensaje,PDO::PARAM_STR);
    $stmt->bindParam(2,$idUsuario_destino,PDO::PARAM_STR);
